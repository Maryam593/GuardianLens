...
const Header = () => {
  ...
  useEffect(() => {
    // Ensure user data updates from localStorage on mount
  ...
  const handleLogout = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setUser(null); // Make sure state updates
      ...
      <div className="flex justify-between items-center px-6 py-4">
        ...
        >
          <ul className="md:flex md:space-x-6">
            <li className="p-3 cursor-pointer transition duration-300" onClick={() => navigate("/")}>
              Home
            </li>
            {user ? (
              <>
                <li className="p-3 cursor-pointer transition duration-300" onClick={() => navigate("/protection/panel")}>
                  Protection Panel
                </li>
              ...
              <li className="p-3 cursor-pointer transition duration-300" onClick={() => navigate("/auth/login")}>
                Login
              </li>
            )}
          </ul>
